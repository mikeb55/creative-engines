...
exports.eventsToMeasureXml = eventsToMeasureXml;
exports.eventsToMeasureXmlWithBackup = eventsToMeasureXmlWithBackup;
...
function eventsToMeasureXml(events, measureIndex, measureStart) {
    ...
    for (const voice of voices) {
        ...
        for (const e of voiceEvents) {
            if (e.startDivision > cursor) {
                ...
                xml += `        <note><rest/><duration>${restDiv}</duration><type>${(0, notationTimingConstants_1.divisionsToType)(restDiv)}</type><voice>${voice}</voice></note>\n`;
                cursor = e.startDivision;
            }
            if (e.rest) {
                xml += `        <note><rest/><duration>${e.durationDivisions}</duration><type>${(0, notationTimingConstants_1.divisionsToType)(e.durationDivisions)}</type><voice>${voice}</voice></note>\n`;
            }
            else {
                ...
                const notations = [];
                if (e.tieStart)
                    notations.push('<tie type="start"/>');
                if (e.tieStop)
                    notations.push('<tie type="stop"/>');
                const notationsEl = notations.length ? `<notations>${notations.join('')}</notations>` : '';
                const staffEl = e.staff ? `<staff>${e.staff}</staff>` : '';
                xml += `        <note><pitch><step>${step}</step>${alterEl}<octave>${octave}</octave></pitch><duration>${e.durationDivisions}</duration><type>${(0, notationTimingConstants_1.divisionsToType)(e.durationDivisions)}</type><voice>${voice}</voice>${staffEl}${notationsEl}</note>\n`;
            }
            cursor += e.durationDivisions;
        }
        if (cursor < measureStart + notationTimingConstants_1.MEASURE_DURATION) {
            const restDiv = measureStart + notationTimingConstants_1.MEASURE_DURATION - cursor;
            xml += `        <note><rest/><duration>${restDiv}</duration><type>${(0, notationTimingConstants_1.divisionsToType)(restDiv)}</type><voice>${voice}</voice></note>\n`;
        }
    }
    return xml;
...
function eventsToMeasureXmlWithBackup(events, measureIndex, measureStart) {
    ...
    let xml = '';
    const slotDuration = notationTimingConstants_1.MEASURE_DURATION / 4; // 4 beats per measure
    for (let slot = 0; slot < 4; slot++) {
        const slotStart = measureStart + slot * slotDuration;
        const slotEnd = slotStart + slotDuration;
        const slotEvents = sorted.filter((e) => e.startDivision >= slotStart && e.startDivision < slotEnd && !e.rest);
        ...
        if (v2) {
            if (v1)
                xml += `    <backup><duration>${slotDuration}</duration></backup>\n`;
