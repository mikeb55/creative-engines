"""Tests for Stravinsky Pulse composer IR."""
...
def test_stravinsky_composer_ir_has_required_fields():
...
def test_stravinsky_validate_composer_ir_valid():
    ir = ComposerIR(title="Valid", seed=0)
    r = validate_composer_ir(ir)
    assert r.valid


def test_stravinsky_validate_composer_ir_invalid_title():
    ir = ComposerIR(title="", seed=0)
    r = validate_composer_ir(ir)
    assert not r.valid
    assert "title" in str(r.errors).lower()
