"""Tests for Stravinsky Pulse section compiler."""
...
def test_stravinsky_compile_composition():
    ir = generate_composer_ir_from_title("Pulse Block", 0)
    comp = compile_composition_from_ir(ir)
    assert comp.sections
    assert comp.title == "Pulse Block"


def test_stravinsky_compile_deterministic():
