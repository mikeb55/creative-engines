"""Tests for Messiaen Colour generator."""
...
def test_messiaen_generate_from_title():
    ir = generate_composer_ir_from_title("Colour Panel", 0)
    assert ir.title == "Colour Panel"
    assert ir.section_order
    assert ir.motivic_cells


def test_messiaen_generate_from_premise():
    ir = generate_composer_ir_from_premise("ecstatic colour study", 0)
    assert ir.title
    assert ir.section_roles


def test_messiaen_generate_deterministic():
...
def test_messiaen_generate_candidates():
    cands = generate_composer_ir_candidates("Colour", count=4, seed=0)
    assert len(cands) == 4
