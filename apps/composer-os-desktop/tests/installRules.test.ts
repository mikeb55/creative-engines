...
import {
  ...
  isComposerOsDesktopShortcutName,
...
describe('installRules', () => {
  ...
  it('isComposerOsDesktopShortcutName is exact match only', () => {
    expect(isComposerOsDesktopShortcutName('Composer OS.lnk')).toBe(true);
    expect(isComposerOsDesktopShortcutName('composer os.lnk')).toBe(true);
    expect(isComposerOsDesktopShortcutName('Composer OS (old).lnk')).toBe(false);
  });
  ...
  it('isLegacyOrForbiddenTarget flags composer-studio paths', () => {
    ...
    expect(isLegacyOrForbiddenTarget('C:\\release\\Composer-OS-1.0.0-portable.exe')).toBe(false);
  });

  it('looksLikeComposerOsPackagedExe recognizes portable / setup patterns', () => {
    expect(looksLikeComposerOsPackagedExe('C:\\r\\Composer-OS-1.0.0-portable.exe')).toBe(true);
  ...
  it('shouldQuarantineShortcut removes legacy names regardless of target', () => {
    const canon = 'C:\\r\\Composer-OS-1.0.0-portable.exe';
  ...
  it('shouldQuarantineShortcut removes Composer OS shortcut when target is not canonical portable', () => {
    const canon = 'C:\\r\\Composer-OS-2.0.0-portable.exe';
    expect(shouldQuarantineShortcut('Composer OS.lnk', 'C:\\old\\Composer-OS-1.0.0-portable.exe', canon)).toBe(
      true
    );
  ...
  it('findCanonicalPortableExe picks newest by mtime', () => {
    ...
    const older = path.join(dir, 'Composer-OS-1.0.0-portable.exe');
    const newer = path.join(dir, 'Composer-OS-2.0.0-portable.exe');
    ...
    expect(name).toBe('Composer-OS-2.0.0-portable.exe');
