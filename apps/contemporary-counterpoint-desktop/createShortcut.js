/**
 * Create Windows desktop shortcut for Contemporary Counterpoint Generator
 * Run: node createShortcut.js
 *
 * Uses a Node launcher (launchContemporaryCounterpointDesktop.js) so paths with spaces work.
...
const shortcutPath = path.join(desktopPath, 'Contemporary Counterpoint Generator.lnk');
const launcherPath = path.join(appDir, 'launchContemporaryCounterpointDesktop.js');
...
oLink.Description = "Contemporary Counterpoint Generator"
...
try {
  ...
  console.log('Desktop shortcut created: Contemporary Counterpoint Generator');
  console.log('Target:', nodeExe);
  console.log('Arguments:', '"' + launcherPath + '"');
  console.log('WorkingDir:', appDir);
} catch (e) {
  console.log('Note: Run "npm start" from apps/contemporary-counterpoint-desktop to launch. Shortcut creation may require cscript.');
} finally {
