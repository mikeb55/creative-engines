/**
 * Create Windows desktop shortcut for Ellington Orchestration Generator
 * Run: node createShortcut.js
 *
 * Uses a Node launcher (launchEllingtonDesktop.js) so paths with spaces work.
 */
...
const shortcutPath = path.join(desktopPath, 'Ellington Orchestration Generator.lnk');
const launcherPath = path.join(appDir, 'launchEllingtonDesktop.js');
...
oLink.Description = "Ellington Orchestration Generator"
...
try {
  ...
  console.log('Desktop shortcut created: Ellington Orchestration Generator');
  console.log('Target:', nodeExe);
  console.log('Arguments:', '"' + launcherPath + '"');
  console.log('WorkingDir:', appDir);
} catch (e) {
  console.log('Note: Run "npm start" from apps/ellington-orchestration-desktop to launch. Shortcut creation may require cscript.');
} finally {
