...
const wyble = spawnSync('npm', ['run', 'wyble'], { cwd: ROOT, stdio: 'inherit' });
if (wyble.status !== 0) {
  console.error('Wyble failed');
  process.exit(1);
}

const counterpoint = spawnSync('npm', ['run', 'counterpoint'], { cwd: ROOT, stdio: 'inherit' });
if (counterpoint.status !== 0) {
  console.error('Counterpoint failed');
  process.exit(1);
}

const ellington = spawnSync('npm', ['run', 'ellington'], { cwd: ROOT, stdio: 'inherit' });
