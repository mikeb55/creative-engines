...
function verifyWyble(filePath) {
    const r = { fileExists: false, fileSize: 0, measureCount: 0, measuresValid: false, voicesValid: false, rangeValid: true, errors: [] };
    ...
    if (r.fileSize <= 0)
        r.errors.push('File size is 0');
    const text = fs.readFileSync(filePath, 'utf8');
    if (!text.includes('<measure'))
        r.errors.push('Missing <measure> tags');
    ...
    if (divisions !== 4)
        r.errors.push(`Expected divisions=4, got ${divisions}`);
    for (const part of parts) {
        if (part.durations.length !== 8)
            r.errors.push(`Part ${part.id}: expected 8 measures, got ${part.durations.length}`);
        for (let i = 0; i < part.durations.length; i++) {
            const dur = part.durations[i];
            if (dur !== expectedPerMeasure)
                r.errors.push(`Part ${part.id} measure ${i + 1}: expected 16, got ${dur}`);
            if (dur === 0)
                r.errors.push(`Part ${part.id} measure ${i + 1}: empty measure`);
        }
    }
...
function verifyCounterpoint(filePath) {
    const r = { fileExists: false, fileSize: 0, measureCount: 0, measuresValid: false, voicesValid: false, rangeValid: true, errors: [] };
    if (!fs.existsSync(filePath)) {
        r.errors.push('File does not exist');
        return r;
...
function verifyEllington(filePath) {
    const r = { fileExists: false, fileSize: 0, measureCount: 0, measuresValid: false, voicesValid: false, rangeValid: true, errors: [] };
    if (!fs.existsSync(filePath)) {
        r.errors.push('File does not exist');
        return r;
    ...
    for (const part of parts) {
        ...
        for (let i = 0; i < part.durations.length; i++) {
            const dur = part.durations[i];
            if (dur !== expectedPerMeasure && dur !== 0)
                r.errors.push(`Part ${part.id} measure ${i + 1}: expected 16 or 0, got ${dur}`);
        }
    }
    ...
    for (const m of pitchMatches) {
        ...
        if (midi < 0 || midi > 127)
            r.errors.push(`Note out of MIDI range: ${midi}`);
    }
