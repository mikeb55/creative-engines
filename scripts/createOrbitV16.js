...
function bassNote(pc, octave, duration, divisions) {
...
function composeBassMeasure(measureNum, chordStr, divisions, prevPc) {
...
function durationToType(d, divs) {
    const quarter = divs;
    const half = divs * 2;
    const dottedHalf = divs * 3;
    const whole = divs * 4;
    if (d <= Math.floor(divs / 2))
        return { type: '16th' };
    if (d <= divs)
        return { type: 'eighth' };
    if (d <= divs * 1.5)
    ...
    if (d <= dottedHalf)
        return { type: 'half' };
    if (d <= dottedHalf + divs / 2)
        return { type: 'half', dot: true };
    return { type: 'whole' };
}
function bassNotesToXml(notes, voice = 1) {
    let xml = '';
...
function harmonyToXml(chordStr) {
    if (!chordStr)
        return '';
    const parsed = parseChordRoot(chordStr);
    if (!parsed)
    ...
    let kind = 'major';
    if (chordStr.includes('maj7') || chordStr.includes('maj'))
        kind = 'major';
    else if (chordStr.includes('7') && !chordStr.includes('maj'))
        kind = 'dominant';
    else if (chordStr.includes('m') || chordStr.includes('min'))
        kind = 'minor-minor';
