...
function parseChord(s: string): { root: number; third: number; seventh: number; bass?: number } | null {
  if (!s) return null;
  const slash = s.indexOf('/');
  if (slash >= 0) {
    const bassStr = s.slice(slash + 1).trim();
    const b = PC[bassStr];
    if (b !== undefined) return { root: b, third: (b + 4) % 12, seventh: (b + 10) % 12, bass: b };
    return null;
  }
...
interface N { rest?: boolean; pc?: number; oct?: number; duration: number; divisions: number; tieStart?: boolean; tieStop?: boolean }
...
function d2type(d: number, divs: number): { type: string; dot?: boolean } {
  ...
  if (d <= dh && d > h) return { type: 'half', dot: true }; // dotted half for full 3/4 bar
  if (d <= dh) return { type: 'half' };
  if (d <= dh + q / 2) return { type: 'half', dot: true };
  return { type: 'whole' };
}

function noteToOctave(pc: number): number {
  if (pc >= 5 && pc <= 11) return 2;
  if (pc >= 0 && pc <= 4) return 2;
  return 2;
}
...
function composeV17Bass(meas: number, chordStr: string, divs: number, prevLastPc: number | null): N[] {
  const total = divs * 3;
  ...
  if (meas >= 18 && meas <= 25) {
    const fPc = 5;
    const fOct = 2;
    if (meas === 18) return [{ pc: fPc, oct: fOct, duration: 4, divisions: divs }, { pc: fPc, oct: fOct, duration: 4, divisions: divs }, { pc: fPc, oct: fOct, duration: 4, divisions: divs }];
    if (meas === 19) return [{ rest: true, duration: 4, divisions: divs }, { pc: fPc, oct: fOct, duration: 8, divisions: divs }];
    if (meas === 20) return [{ pc: fPc, oct: fOct, duration: 6, divisions: divs }, { rest: true, duration: 6, divisions: divs }];
    if (meas === 21) return [{ pc: fPc, oct: fOct, duration: total, divisions: divs }];
    if (meas === 22) return [{ rest: true, duration: 6, divisions: divs }, { pc: fPc, oct: fOct, duration: 6, divisions: divs }];
    if (meas === 23) return [{ pc: fPc, oct: fOct, duration: 4, divisions: divs }, { pc: 9, oct: 2, duration: 4, divisions: divs }, { pc: fPc, oct: fOct, duration: 4, divisions: divs }];
    if (meas === 24) return [{ pc: fPc, oct: fOct, duration: total, divisions: divs }];
    if (meas === 25) return [{ pc: fPc, oct: fOct, duration: 8, divisions: divs }, { pc: fPc, oct: fOct, duration: 4, divisions: divs }];
  ...
  if (meas >= 26) {
    ...
    if (meas === 26) return [{ pc: r, oct: ro, duration: 6, divisions: divs }, { pc: t, oct: ro, duration: 6, divisions: divs }];
    if (meas === 27) {
    ...
    if (meas === 28) return [{ pc: 4, oct: 2, duration: 4, divisions: divs }, { pc: 5, oct: 2, duration: 8, divisions: divs }];
    if (meas === 29) return [{ pc: 5, oct: 2, duration: total, divisions: divs }];
  }
...
function main(): void {
  ...
  const measures: { num: number; content: string }[] = [];
