...
export function validateEllingtonMusicXML(xml: string): { valid: boolean; errors: string[] } {
  const errors: string[] = [];
  ...
  if (!/<part-list>/.test(xml)) errors.push('Required: <part-list>');
  ...
  for (const id of scorePartIds) {
    if (!partIds.includes(id)) errors.push(`Part ${id} in part-list but no matching <part id="${id}">`);
  }
  for (const id of partIds) {
    if (!scorePartIds.includes(id)) errors.push(`Part ${id} present but not in part-list`);
  }
  ...
  while ((mm = measureRegex.exec(xml)) !== null) {
    const content = mm[2];
    const noteDurations = [...content.matchAll(/<note>[\s\S]*?<duration>(\d+)<\/duration>/g)].map((m) =>
      parseInt(m[1], 10)
    );
  ...
  if (malformedNote.test(xml)) errors.push('Found empty <note></note> block');
  ...
  if (!/<divisions>\d+<\/divisions>/.test(xml)) errors.push('Required: <divisions> in attributes');
  if (!/<fifths>/.test(xml)) errors.push('Required: <key><fifths>');
  if (!/<beats>4<\/beats>/.test(xml)) errors.push('Expected 4/4 time');
