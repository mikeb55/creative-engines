...
function parseChord(s) {
    ...
    if (slash >= 0) {
        ...
        const b = PC[bassStr];
        if (b !== undefined)
            return { root: b, third: (b + 4) % 12, seventh: (b + 10) % 12 };
        return null;
    }
 ...
 */
function composeV19Bass(meas, chordStr, divs) {
    const total = divs * 3;
    ...
    if (meas >= 18 && meas <= 25) {
        const f = 5;
        if (meas === 18)
            return [{ pc: f, oct: OCT, duration: 4, divisions: divs }, { pc: f, oct: OCT, duration: 4, divisions: divs }, { pc: f, oct: OCT, duration: 4, divisions: divs }];
        if (meas === 19)
            return [{ rest: true, duration: 4, divisions: divs }, { pc: f, oct: OCT, duration: 8, divisions: divs }];
        if (meas === 20)
            return [{ pc: f, oct: OCT, duration: 6, divisions: divs }, { rest: true, duration: 6, divisions: divs }];
        if (meas === 21)
            return [{ pc: f, oct: OCT, duration: total, divisions: divs }];
        if (meas === 22)
            return [{ rest: true, duration: 6, divisions: divs }, { pc: f, oct: OCT, duration: 6, divisions: divs }];
        if (meas === 23)
            return [{ pc: f, oct: OCT, duration: 4, divisions: divs }, { pc: 9, oct: OCT, duration: 4, divisions: divs }, { pc: f, oct: OCT, duration: 4, divisions: divs }];
        if (meas === 24)
            return [{ pc: f, oct: OCT, duration: total, divisions: divs }];
        if (meas === 25)
            return [{ pc: f, oct: OCT, duration: 8, divisions: divs }, { pc: f, oct: OCT, duration: 4, divisions: divs }];
    }
    // C section: Dbmaj7→Bb7→Fmaj7 — inevitable, composed
    if (meas >= 26) {
        if (meas === 26)
            return [{ pc: 0, oct: OCT, duration: 4, divisions: divs }, { pc: 1, oct: OCT, duration: 8, divisions: divs }]; // C→Db
        if (meas === 27)
            return [{ pc: 10, oct: OCT, duration: 4, divisions: divs }, { pc: 2, oct: OCT, duration: 4, divisions: divs }, { pc: 8, oct: OCT, duration: 4, divisions: divs }]; // Bb D Ab
        if (meas === 28)
            return [{ pc: 4, oct: OCT, duration: 4, divisions: divs }, { pc: 5, oct: OCT, duration: 8, divisions: divs }]; // E→F
        if (meas === 29)
            return [{ pc: 5, oct: OCT, duration: total, divisions: divs }];
    }
    if (!chord)
        return [];
