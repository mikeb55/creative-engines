 ...
 */
function composeV18Bass(meas: number, chordStr: string, divs: number): N[] {
  const total = divs * 3;
  ...
  if (meas >= 18 && meas <= 25) {
    ...
    if (meas === 23) return [{ pc: fPc, oct: fOct, duration: 4, divisions: divs }, { pc: 9, oct: 2, duration: 4, divisions: divs }, { pc: fPc, oct: fOct, duration: 4, divisions: divs }]; // F-A-F motif on pedal
...
function main(): void {
  ...
  out = out.replace(
    /<score-part id="P2">[\s\S]*?<\/score-part>/,
    p2ScorePartWithMidi.trim()
  );
