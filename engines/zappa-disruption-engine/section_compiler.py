"""
Zappa Disruption Section Compiler — Jagged melody, collision harmony.
"""
...
def compile_composition_from_ir(composer_ir: ComposerIR) -> CompiledComposition:
    ...
    for role in composer_ir.section_order:
        ...
        if sp is None:
            sp = SectionPlan(role=role, bar_count=4)
...
def compile_section(sp: SectionPlan, ir: ComposerIR, section_id: str, bar_start: int) -> CompiledSection:
    ...
    if sp.role in ("cut", "contrast"):
        return _compile_cut(sp, ir, section_id, bar_start)
...
def _compile_primary(sp: SectionPlan, ir: ComposerIR, section_id: str, bar_start: int) -> CompiledSection:
    ...
    cell = ir.motivic_cells[0] if ir.motivic_cells else MotivicCell(intervals=[1, 6, 11])
    reg = ir.registral_plan.get("primary", cell.registral_center)
    events = _melody_from_disruption_cell(cell, bars, bar_start, section_id, reg, ir.seed)
...
def _compile_cut(sp: SectionPlan, ir: ComposerIR, section_id: str, bar_start: int) -> CompiledSection:
    bars = sp.bar_count or 3
    bar_end = bar_start + bars
    cell = ir.motivic_cells[1] if len(ir.motivic_cells) > 1 else (ir.motivic_cells[0] if ir.motivic_cells else MotivicCell(intervals=[1, 6, 11]))
    developed = develop_motif_cell(cell, "interruption")
    reg = ir.registral_plan.get("cut", cell.registral_center + 10)
    events = _melody_from_disruption_cell(developed, bars, bar_start, section_id, reg, ir.seed + 1)
    harm = derive_section_harmony(ir.harmonic_field, "cut")
    harmony = [{"symbol": harm[i % len(harm)], "measure": bar_start + i, "duration": 4} for i in range(bars)]
    pl = sp.phrase_lengths or [2, 1]
    return CompiledSection(section_id=section_id, role="cut", bar_start=bar_start, bar_end=bar_end, melody_events=events, harmony=harmony, phrase_lengths=pl, motif_refs=["m2"], register_hint=reg)
...
def _compile_return(sp: SectionPlan, ir: ComposerIR, section_id: str, bar_start: int) -> CompiledSection:
    ...
    bar_end = bar_start + bars
    cell = ir.motivic_cells[0] if ir.motivic_cells else MotivicCell(intervals=[1, 6, 11])
    developed = develop_motif_cell(cell, "sudden_reentry")
    reg = ir.registral_plan.get("return", cell.registral_center)
    events = _melody_from_disruption_cell(developed, bars, bar_start, section_id, reg, ir.seed + 2)
...
def _compile_generic(sp: SectionPlan, ir: ComposerIR, section_id: str, bar_start: int) -> CompiledSection:
    ...
    bar_end = bar_start + bars
    cell = ir.motivic_cells[0] if ir.motivic_cells else MotivicCell(intervals=[1, 6, 11])
    events = _melody_from_disruption_cell(cell, bars, bar_start, section_id, cell.registral_center, ir.seed)
    harm = derive_section_harmony(ir.harmonic_field, "primary")
    harmony = [{"symbol": harm[i % len(harm)], "measure": bar_start + i, "duration": 4} for i in range(bars)]
...
def _melody_from_disruption_cell(cell: MotivicCell, bars: int, bar_start: int, section_id: str, reg: int, seed: int) -> List[Dict]:
    """Jagged disruption melody: abrupt, cut-aware."""
    events = []
    pitch = reg
    total_beats = bars * 4
    h = _hash_int(seed)
