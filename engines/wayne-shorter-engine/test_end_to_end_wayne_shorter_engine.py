"""End-to-end tests for Wayne Shorter engine."""
...
def test_title_to_compile_to_musicxml():
    ir = generate_composer_ir_from_title("Footprints", seed=42)
    comp = compile_composition_from_ir(ir)
    xml = export_composition_to_musicxml(comp)
    assert "Footprints" in xml
    assert "</score-partwise>" in xml


def test_premise_to_compile_to_musicxml():
    ir = generate_composer_ir_from_premise("A dark modal melody", seed=17)
    comp = compile_composition_from_ir(ir)
    xml = export_composition_to_musicxml(comp)
...
def test_all_examples_compile():
    ...
    for c in comps:
        ...
        assert sum(len(s.melody_events) for s in c.sections) > 0
...
def test_all_examples_export():
    ...
    for c in comps:
        ...
        assert "<score-partwise" in xml
        assert "<work-title>" in xml and "</work-title>" in xml
