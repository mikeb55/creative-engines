...
const MODES = ['classic', 'ballad', 'shout'] as const;
const MIN_TESTS = 100;
...
function scoreMusicalUsefulness(plan: { suggestions: ArrangerSuggestion[] }): number {
...
function scoreSectionFit(plan: { suggestions: ArrangerSuggestion[] }, sectionNames: string[]): number {
...
function scoreDensityAwareness(plan: { suggestions: ArrangerSuggestion[] }): number {
  const densities = new Set(plan.suggestions.map((s) => s.density));
  return Math.min(10, 6 + densities.size * 1.2);
}

function scorePhraseAwareness(plan: { suggestions: ArrangerSuggestion[] }): number {
...
function scoreVariety(plan: { suggestions: ArrangerSuggestion[] }): number {
...
function scoreSibeliusUsefulness(plan: { suggestions: ArrangerSuggestion[] }): number {
...
function validateBackgroundsNoOverrideLead(plan: { suggestions: ArrangerSuggestion[] }): boolean {
  const backgrounds = plan.suggestions.filter((s) => s.role === 'background_figure');
  for (const b of backgrounds) {
    if (b.description.toLowerCase().includes('override') || b.description.toLowerCase().includes('lead melody')) return false;
  }
  return true;
}

function validatePunctuationPlausible(plan: { suggestions: ArrangerSuggestion[] }): boolean {
  const punct = plan.suggestions.filter((s) => s.role === 'punctuation');
  for (const p of punct) {
    if (p.barRange.endBar < p.barRange.startBar) return false;
    if (p.barRange.startBar < 1) return false;
  }
  return true;
}

function validateSoliFitsSoli(plan: { suggestions: ArrangerSuggestion[] }, sectionRoles: string[]): boolean {
  const soli = plan.suggestions.filter((s) => s.role === 'soli_texture');
  const hasSoliSection = sectionRoles.includes('soli');
  if (soli.length > 0 && !hasSoliSection) return true;
  return true;
}

function validateShoutRampIntensifies(plan: { suggestions: ArrangerSuggestion[] }): boolean {
  const ramps = plan.suggestions.filter((s) => s.role === 'shout_ramp') as any[];
  const order = ['setup', 'intensification', 'arrival', 'release'];
  let prevIdx = -1;
  for (const r of ramps) {
    const idx = order.indexOf(r.subtype);
    if (idx >= 0 && idx < prevIdx) return false;
    if (idx >= 0) prevIdx = idx;
  }
  return true;
}

function scorePlan(plan: { suggestions: ArrangerSuggestion[] }, sectionNames: string[], sectionRoles: string[]): number {
  if (!validateBackgroundsNoOverrideLead(plan)) return 0;
  if (!validatePunctuationPlausible(plan)) return 0;
  if (!validateShoutRampIntensifies(plan)) return 0;
...
function main(): void {
  ...
  const beatricePlans: { score: number; plan: any }[] = [];
  const orbitPlans: { score: number; plan: any }[] = [];
  ...
  for (const templateId of TEMPLATE_IDS) {
    ...
    for (const mode of MODES) {
      for (let i = 0; i < 8; i++) {
        ...
        const architecture = generateArchitecture(template.segments, {
        ...
        if (templateId === 'beatrice_A') beatricePlans.push({ score: sc, plan: assistPlan });
        if (templateId === 'orbit_A') orbitPlans.push({ score: sc, plan: assistPlan });
      }
    }
  }
  ...
  while (avgScore < TARGET_AVG && iter < 3) {
    ...
    for (const templateId of TEMPLATE_IDS) {
      ...
      for (let i = 0; i < 4; i++) {
        ...
        const assistPlan = generateArrangerAssist(architecture, ellingtonPlan, { seed: seed + 2 });
  ...
  for (let i = 0; i < beatriceTop.length; i++) {
    fs.writeFileSync(
      path.join(exportDir, `beatrice_top${i + 1}.json`),
      JSON.stringify(beatriceTop[i].plan, null, 2),
      'utf-8'
    );
  }
  for (let i = 0; i < orbitTop.length; i++) {
    fs.writeFileSync(
      path.join(exportDir, `orbit_top${i + 1}.json`),
      JSON.stringify(orbitTop[i].plan, null, 2),
      'utf-8'
    );
  }
}
