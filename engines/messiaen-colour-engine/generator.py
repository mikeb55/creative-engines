"""
Messiaen Colour Generator — Assemble ComposerIR from title/premise.
"""
...
PROFILES = ["birdsong_fragment", "luminous_fourths", "mode_coloured_seconds", "ecstatic_leaps", "chromatic_colour_cluster"]
HARMONY_PROFILES = ["mode_2_field", "mode_3_field", "colour_chord_field", "radiant_axis", "suspended_ecstatic", "static_luminous_center"]
FORM_PROFILES = ["colour_panels", "ecstatic_arc", "asymmetrical_radiance", "birdsong_interruption_form", "static_center_transformed_return"]
...
def generate_composer_ir_from_title(title: str, seed: int = 0, profile: str = "birdsong_fragment") -> ComposerIR:
    ...
    ir = ComposerIR(
        title=title,
        seed=seed,
        tempo_hint=66 + (seed % 20),
        form_plan=form.get("profile", "colour_panels"),
...
def generate_composer_ir_from_premise(premise: str, seed: int = 0, profile: str = "birdsong_fragment") -> ComposerIR:
...
def generate_composer_ir_candidates(input_text: str, mode: str = "title", count: int = 12, seed: int = 0) -> List[ComposerIR]:
    ...
    for i in range(count):
        ...
        try:
            ...
            ir = generate_composer_ir_from_title("Untitled", s, "birdsong_fragment")
            out.append(ir)
    return out[:count]
