"""
Wheeler Lyric Form Planner — Long arc, asymmetric, transformed returns.
"""
...
PROFILES = {
    "lyrical_songform": ([5, 7, 5, 7], ["primary", "contrast", "return"]),
    "asymmetrical_arc": ([7, 9, 5], ["primary", "contrast", "return"]),
    "bridge_lift_return": ([5, 7, 9, 5], ["primary", "contrast", "return"]),
    "floating_refrain": ([7, 5, 7, 6], ["primary", "contrast", "return"]),
    "chamber_ballad_form": ([9, 7, 9], ["primary", "contrast", "return"]),
}
...
def plan_form(seed: int, profile: str = "lyrical_songform") -> Dict[str, Any]:
    """Plan form. Phrase groups 5–7, 7–9, 9–11 bars. Deterministic."""
    prof = profile
    phrase_lengths, section_order = PROFILES.get(prof, PROFILES["lyrical_songform"])
    h = _hash_int(seed)
