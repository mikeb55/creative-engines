"""
Wheeler Lyric Generator — Assemble ComposerIR from title/premise.
"""
...
HARMONY_PROFILES = ["suspended_lyric", "floating_modal", "open_tonal_center", "major_minor_shade", "chamber_ecm", "soft_axis"]
FORM_PROFILES = ["lyrical_songform", "asymmetrical_arc", "bridge_lift_return", "floating_refrain", "chamber_ballad_form"]
...
def generate_composer_ir_from_title(title: str, seed: int = 0, profile: str = "lyrical_wide") -> ComposerIR:
    ...
    ir = ComposerIR(
        title=title,
        seed=seed,
        tempo_hint=60 + (seed % 40),
        form_plan=form.get("profile", "lyrical_songform"),
        ...
        phrase_plan=pp,
        contrast_plan=ContrastPlan(section_contrasts={"contrast": 0.6, "return": 0.85}),
...
def generate_composer_ir_from_premise(premise: str, seed: int = 0, profile: str = "lyrical_wide") -> ComposerIR:
...
def generate_composer_ir_candidates(input_text: str, mode: str = "title", count: int = 12, seed: int = 0) -> List[ComposerIR]:
    ...
    for i in range(count):
        ...
        try:
            ...
            ir = generate_composer_ir_from_title("Untitled", s, "lyrical_wide")
            out.append(ir)
    return out[:count]
