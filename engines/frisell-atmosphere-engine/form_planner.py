"""
Frisell Atmosphere Form Planner — Open, floating, asymmetric, spacious.
"""
...
PROFILES = {
    "open_songform": ([5, 7, 5, 6], ["primary", "contrast", "return"]),
    "floating_refrain": ([7, 5, 7, 6], ["primary", "contrast", "return"]),
    "atmospheric_arc": ([6, 8, 6], ["primary", "contrast", "return"]),
    "asymmetrical_ballad": ([7, 5, 9, 5], ["primary", "contrast", "return"]),
    "spacious_return": ([5, 7, 7, 5], ["primary", "contrast", "return"]),
}
...
def plan_form(seed: int, profile: str = "open_songform") -> Dict[str, Any]:
    """Plan form. Asymmetric, spacious. Deterministic."""
    prof = profile
    phrase_lengths, section_order = PROFILES.get(prof, PROFILES["open_songform"])
    h = _hash_int(seed)
