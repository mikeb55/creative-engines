"""
Frisell Atmosphere Generator — Assemble ComposerIR from title/premise.
"""
...
HARMONY_PROFILES = ["pedal_field", "americana_open", "floating_major_minor", "suspended_plain", "ambient_modal"]
FORM_PROFILES = ["open_songform", "floating_refrain", "atmospheric_arc", "asymmetrical_ballad", "spacious_return"]
...
def generate_composer_ir_from_title(title: str, seed: int = 0, profile: str = "open_fifths") -> ComposerIR:
    ...
    ir = ComposerIR(
        title=title,
        seed=seed,
        tempo_hint=55 + (seed % 35),
        form_plan=form.get("profile", "open_songform"),
...
def generate_composer_ir_from_premise(premise: str, seed: int = 0, profile: str = "open_fifths") -> ComposerIR:
...
def generate_composer_ir_candidates(input_text: str, mode: str = "title", count: int = 12, seed: int = 0) -> List[ComposerIR]:
    ...
    for i in range(count):
        ...
        try:
            ...
            ir = generate_composer_ir_from_title("Untitled", s, "open_fifths")
            out.append(ir)
    return out[:count]
