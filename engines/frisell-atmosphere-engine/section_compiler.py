"""
Frisell Atmosphere Section Compiler — Sparse lyric motifs, pedal harmony, spacious.
"""
...
def compile_primary_theme(section_plan: SectionPlan, composer_ir: ComposerIR, section_id: str, bar_start: int) -> CompiledSection:
    ...
    events = _melody_from_atmosphere_cells(cell, bars, bar_start, section_id, reg, composer_ir.seed)
...
def compile_contrast_section(section_plan: SectionPlan, composer_ir: ComposerIR, section_id: str, bar_start: int) -> CompiledSection:
    ...
    developed = develop_motif_cell(cell, "registral_drift")
    reg = composer_ir.registral_plan.get("contrast", cell.registral_center + 5)
    events = _melody_from_atmosphere_cells(developed, bars, bar_start, section_id, reg, composer_ir.seed + 1)
    harm = derive_section_harmony(composer_ir.harmonic_field, "contrast")
    harmony = [{"symbol": harm[i % len(harm)], "measure": bar_start + i, "duration": 4} for i in range(bars)]
    pl = section_plan.phrase_lengths or [3, 2]
    return CompiledSection(section_id=section_id, role="contrast", bar_start=bar_start, bar_end=bar_end, melody_events=events, harmony=harmony, phrase_lengths=pl, motif_refs=["m2_drift"], register_hint=reg)
...
def compile_return_section(section_plan: SectionPlan, composer_ir: ComposerIR, section_id: str, bar_start: int) -> CompiledSection:
    ...
    bar_end = bar_start + bars
    cell = composer_ir.motivic_cells[0] if composer_ir.motivic_cells else MotivicCell(intervals=[5, 7, -4])
    developed = develop_motif_cell(cell, "harmonic_shadow")
    reg = composer_ir.registral_plan.get("return", cell.registral_center - 5)
    events = _melody_from_atmosphere_cells(developed, bars, bar_start, section_id, reg, composer_ir.seed + 2)
    harm = derive_section_harmony(composer_ir.harmonic_field, "primary")
    harmony = [{"symbol": harm[i % len(harm)], "measure": bar_start + i, "duration": 4} for i in range(bars)]
    pl = section_plan.phrase_lengths or [4, 3]
    return CompiledSection(section_id=section_id, role="return", bar_start=bar_start, bar_end=bar_end, melody_events=events, harmony=harmony, phrase_lengths=pl, motif_refs=["m1_shadow"], register_hint=reg)
...
def _compile_generic_section(section_plan: SectionPlan, composer_ir: ComposerIR, section_id: str, bar_start: int) -> CompiledSection:
    ...
    bar_end = bar_start + bars
    cell = composer_ir.motivic_cells[0] if composer_ir.motivic_cells else MotivicCell(intervals=[5, 7, -4])
    events = _melody_from_atmosphere_cells(cell, bars, bar_start, section_id, cell.registral_center, composer_ir.seed)
    harm = derive_section_harmony(composer_ir.harmonic_field, "primary")
    harmony = [{"symbol": harm[i % len(harm)], "measure": bar_start + i, "duration": 4} for i in range(bars)]
...
def _melody_from_atmosphere_cells(cell: MotivicCell, bars: int, bar_start: int, section_id: str, reg: int, seed: int) -> List[Dict]:
    """Sparse lyric-like melody: spacious, pedal feel, melodic afterglow."""
    events = []
    pitch = reg
    total_beats = bars * 4
    h = _hash_int(seed)
    note_dur = 1.25 + (h % 2) * 0.5
    gap = 1.0
    beat = 0.0
