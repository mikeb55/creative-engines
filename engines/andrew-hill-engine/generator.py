"""
Hill Generator — Assemble ComposerIR from title/premise.
"""

from typing import List

try:
    from .composer_ir import ComposerIR, PhrasePlan, ContrastPlan, DevelopmentPlan, SectionPlan
...
PROFILES = ["angular", "wide_angular", "cluster_adjacent"]
HARMONY_PROFILES = ["cluster_based", "ambiguous_modal", "nonfunctional_cycle", "pedal_center"]
FORM_PROFILES = ["irregular_5_7_5", "asymmetrical_7_9", "sectional_contrast", "floating_odd"]
...
def generate_composer_ir_from_title(title: str, seed: int = 0, profile: str = "angular") -> ComposerIR:
    ...
    for i, role in enumerate(section_order):
        remaining = len(pl_all) - idx
        phrases_this = max(1, remaining // (n - i)) if i < n - 1 else remaining
        pl = pl_all[idx:idx + phrases_this] if idx < len(pl_all) else [4]
        if not pl:
            pl = [4]
        idx += len(pl)
        bar_count = sum(pl)
    ...
    ir = ComposerIR(
        title=title,
        seed=seed,
        tempo_hint=80 + (seed % 30),
        form_plan=form.get("profile", "irregular_5_7_5"),
        ...
        phrase_plan=pp,
        contrast_plan=ContrastPlan(section_contrasts={"contrast": 0.7, "return": 0.8}),
...
def generate_composer_ir_from_premise(premise: str, seed: int = 0, profile: str = "angular") -> ComposerIR:
...
def generate_composer_ir_candidates(input_text: str, mode: str = "title", count: int = 12, seed: int = 0) -> List[ComposerIR]:
    ...
    for i in range(count):
        ...
        try:
            ...
            ir = generate_composer_ir_from_title("Untitled", s, "angular")
            out.append(ir)
    return out[:count]
