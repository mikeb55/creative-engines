...
exports.validateBacharachConformance = validateBacharachConformance;
function countChromaticSteps(pitches) {
    let n = 0;
...
function validateBacharachConformance(score) {
    const errors = [];
    ...
    for (const m of guitar.measures) {
        for (const e of m.events) {
            if (e.kind === 'note') {
                ...
                starts.push(sb);
                timed.push({ t: (m.index - 1) * 4 + sb, pitch: e.pitch });
            }
        }
    }
    if (starts.length === 0) {
    ...
    if (intervals > 0) {
        ...
        if (ratio === 0 && intervals > 4)
            errors.push('Bacharach: no subtle chromatic colour detected');
    }
