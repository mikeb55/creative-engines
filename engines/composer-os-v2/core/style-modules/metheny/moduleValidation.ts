...
export interface MethenyValidationResult {
  valid: boolean;
  errors: string[];
}

function countScalarSteps(pitches: number[]): number {
  let scalar = 0;
...
export function validateMethenyConformance(score: ScoreModel): MethenyValidationResult {
  const errors: string[] = [];
  ...
  if (guitar) {
    ...
    if (onBeats.length > 0 && square / onBeats.length > 0.9) errors.push('Phrases too square');
  }
