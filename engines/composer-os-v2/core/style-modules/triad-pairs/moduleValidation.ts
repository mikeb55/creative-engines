...
export interface TriadPairValidationResult {
  valid: boolean;
  errors: string[];
}

function hasTriadCellIdentity(pitches: number[]): boolean {
...
function isScalarRun(pitches: number[]): boolean {
  if (pitches.length < 4) return false;
  let stepwise = 0;
  for (let i = 1; i < pitches.length; i++) {
    if (Math.abs(pitches[i] - pitches[i - 1]) <= 2) stepwise++;
  }
  return stepwise >= pitches.length - 1;
}

export function validateTriadPairConformance(score: ScoreModel): TriadPairValidationResult {
  const errors: string[] = [];
  ...
  if (guitar) {
    ...
    for (const m of guitar.measures) {
      for (const e of m.events) {
        if (e.kind === 'note') all.push(e.pitch);
      }
    }
    if (all.length >= 4 && isScalarRun(all)) errors.push('Output scalar/exercise-like');

    const perBar: number[][] = [];
    for (const m of guitar.measures) {
      const bar: number[] = [];
      for (const e of m.events) {
        if (e.kind === 'note') bar.push(e.pitch);
      }
      if (bar.length >= 2 && hasTriadCellIdentity(bar)) return { valid: true, errors: [] };
      perBar.push(bar);
    }
    const anyTriad = perBar.some((bar) => bar.length >= 2 && hasTriadCellIdentity(bar));
    if (!anyTriad && all.length >= 6) errors.push('No triad-cell identity');
    ...
    if (onBeats.length > 4 && offbeat / onBeats.length < 0.1) errors.push('Rhythm too square');
  }
