...
import type { CompositionRequest, GenerationResult } from './conductorTypes';

const VALIDATION_COUNT = 50;
const TARGET_AVERAGE = 9.0;

const STYLES: CompositionRequest['style'][] = ['chamber_jazz', 'big_band', 'guitar_duo'];
const FORMS: CompositionRequest['form'][] = ['AABA', 'blues', 'rhythm_changes'];
const TEMPLATES = ['ii_V_I_major', 'jazz_blues', 'rhythm_changes_A', 'beatrice_A', 'orbit_A'];

function scoreGeneration(result: GenerationResult): number {
  let score = 0;
  const arch = result.architecture;

  if (arch.formStructure && arch.formStructure.sections.length > 0) score += 2;
  if (arch.progression && arch.progression.length > 0) score += 2;
  const orchCall = arch.engineCalls.find((c) => c.engine === 'ellington' || c.engine === 'big_band_architecture');
  if (orchCall?.success) score += 2;
  if (arch.architecturePlan && arch.architecturePlan.sections.length > 0) score += 2;
  if (result.compositionPlanPath || result.architectureJsonPath || result.scoreMusicPath) score += 2;
...
function runValidation(): boolean {
  ...
  for (let i = 0; i < VALIDATION_COUNT; i++) {
    ...
    try {
      ...
      scores.push(s);
      if (s < 9) failures.push(`Comp ${i + 1}: score ${s}`);
    } catch (e) {
      scores.push(0);
...
export { runValidation, scoreGeneration };
