...
const architectureTemplates_1 = require("./architectureTemplates");
function getBaseTemplate(style) {
    switch (style) {
        case 'ellington_style':
            return architectureTemplates_1.ELLINGTON_STYLE.map((s) => ({ ...s }));
        case 'ballad_form':
            return architectureTemplates_1.BALLAD_FORM.map((s) => ({ ...s }));
        default:
            return architectureTemplates_1.STANDARD_SWING.map((s) => ({ ...s }));
    }
}
function scaleTemplateToBars(template, targetBars) {
...
function generateArchitecture(progression, parameters = {}) {
