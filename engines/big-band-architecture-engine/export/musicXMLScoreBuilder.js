...
exports.buildMusicXML = buildMusicXML;
const scoreLayout_1 = require("./scoreLayout");
...
function parseChordForHarmony(chord) {
    const c = chord.trim().replace(/\s+/g, '');
    if (!c)
        return { root: 'C', alter: 0, kind: 'major', text: 'C' };
...
function buildMusicXML(skeleton, staffLayout = scoreLayout_1.BIG_BAND_STAFF_LAYOUT) {
    ...
    for (const r of rehearsalMarks)
        rehearsalByBar.set(r.bar, r.label);
    const sectionByBar = new Map();
    for (const s of sectionLabels)
        sectionByBar.set(s.bar, s.text);
    const cuesByBar = new Map();
    ...
    for (const h of chordSymbols)
        chordByBar.set(h.bar, h.chord);
