...
import type { ChordSegment } from '../architectureTypes';
...
function validateMusicXML(xml: string): { pass: boolean; errors: string[] } {
  const errors: string[] = [];
  if (!xml.includes('<score-partwise')) errors.push('Missing score-partwise');
  if (!xml.includes('<part-list>')) errors.push('Missing part-list');
  if (!xml.includes('</part-list>')) errors.push('Missing part-list close');
  const partCount = (xml.match(/<part id="/g) || []).length;
  if (partCount !== STAFF_COUNT) errors.push(`Expected ${STAFF_COUNT} parts, got ${partCount}`);
  ...
  if (!rehearsalMatch || rehearsalMatch.length === 0) errors.push('No rehearsal marks');
  const harmonyMatch = xml.match(/<harmony>/g);
  if (!harmonyMatch || harmonyMatch.length === 0) errors.push('No chord symbols');
  return { pass: errors.length === 0, errors };
}

function runOneExport(templateId: string, style: string, seed: number): number {
  const template = TEMPLATE_LIBRARY[templateId];
  if (!template) return 0;

  const progression = template.segments;
  const architecture = generateArchitecture(progression, { style: style as any, seed, progressionTemplate: templateId });
  ...
  exportScoreSkeleton(architecture, arrangementPlan, testDir);
  ...
  if (sectionCount !== architecture.sections.length) errors.push('Section count mismatch');
