"""
BH Section Compiler — Melody from harmonic motion, voice-leading priority.
"""
...
def compile_primary_theme(section_plan: SectionPlan, composer_ir: ComposerIR, section_id: str, bar_start: int) -> CompiledSection:
    ...
    cell = composer_ir.motivic_cells[0] if composer_ir.motivic_cells else MotivicCell(intervals=[1, 2, -1, -2])
...
def compile_contrast_section(section_plan: SectionPlan, composer_ir: ComposerIR, section_id: str, bar_start: int) -> CompiledSection:
    ...
    bar_end = bar_start + bars
    cell = composer_ir.motivic_cells[1] if len(composer_ir.motivic_cells) > 1 else (composer_ir.motivic_cells[0] if composer_ir.motivic_cells else MotivicCell(intervals=[1, 2, -1]))
    developed = develop_motif_cell(cell, "enclosure")
    reg = composer_ir.registral_plan.get("contrast", cell.registral_center - 2)
    ...
    harmony = [{"symbol": harm[i % len(harm)], "measure": bar_start + i, "duration": 4} for i in range(bars)]
    pl = section_plan.phrase_lengths or [4]
    return CompiledSection(section_id=section_id, role="contrast", bar_start=bar_start, bar_end=bar_end, melody_events=events, harmony=harmony, phrase_lengths=pl, motif_refs=["m2_enc"], register_hint=reg)
...
def compile_return_section(section_plan: SectionPlan, composer_ir: ComposerIR, section_id: str, bar_start: int) -> CompiledSection:
    ...
    bar_end = bar_start + bars
    cell = composer_ir.motivic_cells[0] if composer_ir.motivic_cells else MotivicCell(intervals=[1, 2, -1, -2])
    developed = develop_motif_cell(cell, "scalar_embellish")
    reg = composer_ir.registral_plan.get("return", cell.registral_center + 3)
    events = _melody_from_cell(developed, bars, bar_start, section_id, reg)
    harm = derive_section_harmony(composer_ir.harmonic_field, "primary")
    harmony = [{"symbol": harm[i % len(harm)], "measure": bar_start + i, "duration": 4} for i in range(bars)]
    pl = section_plan.phrase_lengths or [4, 4]
    return CompiledSection(section_id=section_id, role="return", bar_start=bar_start, bar_end=bar_end, melody_events=events, harmony=harmony, phrase_lengths=pl, motif_refs=["m1_emb"], register_hint=reg)
...
def _melody_from_cell(cell: MotivicCell, bars: int, bar_start: int, section_id: str, reg: int) -> List[Dict]:
    ...
    while beat < total_beats:
        ...
        pitch = max(48, min(84, pitch + inc))
        measure = bar_start + int(beat // 4)
        events.append({"pitch": pitch, "duration": 1.0, "measure": measure, "beat_position": beat % 4, "section_id": section_id})
        idx += 1
        beat += 1.0
    return events
