"""
BH Generator — Assemble ComposerIR from title/premise.
"""
...
PROFILES = ["bebop_step", "enclosure_heavy", "scalar_embellish"]
HARMONY_PROFILES = ["major6_dim", "minor6_dim", "dominant_dim", "minor_conversion"]
FORM_PROFILES = ["compact_8", "aaba_16", "blues_12", "rhythm_32"]
...
def generate_composer_ir_from_title(title: str, seed: int = 0, profile: str = "bebop_step") -> ComposerIR:
    ...
    ir = ComposerIR(
        title=title,
        seed=seed,
        tempo_hint=160 + (seed % 40),
        form_plan=form.get("profile", "aaba_16"),
        ...
        phrase_plan=pp,
        contrast_plan=ContrastPlan(section_contrasts={"contrast": 0.5, "return": 0.7}),
        development_plan=DevelopmentPlan(section_operations=dev),
        cadence_strategy="resolved",
    )
...
def generate_composer_ir_from_premise(premise: str, seed: int = 0, profile: str = "bebop_step") -> ComposerIR:
...
def generate_composer_ir_candidates(input_text: str, mode: str = "title", count: int = 12, seed: int = 0) -> List[ComposerIR]:
    ...
    for i in range(count):
        ...
        try:
            ...
            ir = generate_composer_ir_from_title("Untitled", s, "bebop_step")
            out.append(ir)
    return out[:count]
