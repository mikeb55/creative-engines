"""
Scofield Holland Generator — Assemble ComposerIR from title/premise.
"""
...
PROFILES = ["chromatic_riff", "blues_modern", "groove_cell", "angular_funk", "short_burst_repeat"]
HARMONY_PROFILES = ["funk_blues_modern", "chromatic_dominant", "pedal_groove_field", "minor_blues_shadow", "quartal_funk", "bass_axis_motion"]
FORM_PROFILES = ["groove_head_form", "riff_aaba", "asymmetrical_small_group", "vamp_bridge_return", "chromatic_blues_arc"]
...
def generate_composer_ir_from_title(title: str, seed: int = 0, profile: str = "chromatic_riff") -> ComposerIR:
    ...
    for i, role in enumerate(section_order):
        ...
        pl = pl_all[idx:idx + phrases_per] if idx < len(pl_all) else [6]
        if not pl:
            pl = [6]
        idx += len(pl)
        bar_count = sum(pl)
    ...
    ir = ComposerIR(
        title=title,
        seed=seed,
        tempo_hint=90 + (seed % 30),
        form_plan=form.get("profile", "groove_head_form"),
        ...
        asymmetry_profile="pocket",
        phrase_plan=pp,
        development_plan=DevelopmentPlan(section_operations=dev),
        cadence_strategy="groove_resolution",
    )
...
def generate_composer_ir_from_premise(premise: str, seed: int = 0, profile: str = "chromatic_riff") -> ComposerIR:
...
def generate_composer_ir_candidates(input_text: str, mode: str = "title", count: int = 12, seed: int = 0) -> List[ComposerIR]:
    ...
    for i in range(count):
        ...
        try:
            ...
            ir = generate_composer_ir_from_title("Untitled", s, "chromatic_riff")
            out.append(ir)
    return out[:count]
