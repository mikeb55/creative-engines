...
exports.buildSelectiveMusicXML = buildSelectiveMusicXML;
const STAFF_LAYOUT = [
...
function buildSelectiveMusicXML(plan, totalBars) {
    const divisions = 4;
    const eventsByStaffBar = new Map();
    for (const unit of plan.units) {
        for (const ev of unit.noteEvents ?? []) {
            const key = ev.staffId;
            if (!eventsByStaffBar.has(key))
                eventsByStaffBar.set(key, new Map());
    ...
    for (const staff of STAFF_LAYOUT) {
        ...
        for (let m = 1; m <= totalBars; m++) {
            ...
            if (evs.length === 0) {
                ...
                let offset = 0;
                for (const ev of evs.sort((a, b) => a.beat - b.beat)) {
