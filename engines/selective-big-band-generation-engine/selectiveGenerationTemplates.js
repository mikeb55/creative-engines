...
exports.createBackgroundFigure = createBackgroundFigure;
exports.createBrassPunctuation = createBrassPunctuation;
exports.createSaxSoliTexture = createSaxSoliTexture;
exports.createShoutRampMaterial = createShoutRampMaterial;
...
function chordToSemitones(chord) {
...
function semitoneToPitch(semi, octave) {
    const name = PITCH_NAMES[semi % 12];
    const oct = octave + Math.floor(semi / 12);
    return `${name}${oct}`;
}
function createBackgroundFigure(section, startBar, length, chord, density, staffIds, rnd) {
    ...
    for (let b = 0; b < length; b++) {
        ...
        if (rnd() < 0.6)
            events.push({ bar, beat: 2, duration: 4, pitch: pitch2, staffId: staffIds[1] ?? 'P3' });
    }
    return {
...
function createBrassPunctuation(section, startBar, chord, staffIds, rnd) {
    const semitones = chordToSemitones(chord);
...
function createSaxSoliTexture(section, startBar, length, chord, staffIds, rnd) {
    const semitones = chordToSemitones(chord);
    const events = [];
    for (let b = 0; b < length; b++) {
        const bar = startBar + b;
...
function createShoutRampMaterial(section, startBar, length, chord, phase, staffIds, rnd) {
    const semitones = chordToSemitones(chord);
    const events = [];
