"""
Shorter Form Composer IR Validator — Narrative arc, transformation, asymmetry.
"""
...
def validate_form_plan(composer_ir: Any) -> ValidationResult:
    """Validate narrative arc coherence, section transformation rules, phrase asymmetry."""
    ...
    trans = getattr(composer_ir, "section_transformation_map", {})
    for k in trans:
        if k not in order and k != "return":
            r.warnings.append(f"section_transformation_map key '{k}' not in section_order")
    pp = getattr(composer_ir, "phrase_plan", None)
    if pp and hasattr(pp, "phrase_lengths") and pp.phrase_lengths:
        if not all(L >= 1 for L in pp.phrase_lengths):
            r.valid = False
            r.errors.append("phrase_lengths must be >= 1")
    asym = getattr(composer_ir, "asymmetry_profile", "")
    if asym and asym != "explicit":
        r.warnings.append("asymmetry_profile should be 'explicit' for Shorter form")
    return r
...
def validate_phrase_plan(composer_ir: Any) -> ValidationResult:
    """Validate phrase plan, motif development possible."""
    r = ValidationResult(valid=True)
    pp = getattr(composer_ir, "phrase_plan", None)
    ...
    cells = getattr(composer_ir, "motivic_cells", [])
    if not cells and getattr(composer_ir, "development_plan", None):
        r.warnings.append("motivic_cells empty but development_plan present")
    return r
