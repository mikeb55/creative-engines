"""
Stravinsky Pulse Engine Adapter — ComposerEngine interface.
"""
...
class StravinskyPulseEngine(ComposerEngine):
    engine_name = "stravinsky_pulse"
    supported_profiles = ["pulse_fifth", "sharp_second", "block_fourth", "dry_leap_cell", "repeated_accent_pitch"]
    supported_forms = ["block_contrast", "asymmetrical_ostinato", "pulse_arc", "sectional_refrain", "cut_return"]

    def generate_ir(self, input_text: str, mode: str = "title", seed: int = 0, **kwargs) -> Any:
        if mode == "premise":
            return generate_composer_ir_from_premise(input_text or "Untitled", seed, kwargs.get("profile", "pulse_fifth"))
        return generate_composer_ir_from_title(input_text or "Untitled", seed, kwargs.get("profile", "pulse_fifth"))
...
register_engine("stravinsky_pulse", StravinskyPulseEngine)
