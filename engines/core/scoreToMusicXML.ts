...
function voiceEventsToXml(events: NoteEvent[], voice: number, measureStart: number, staff?: number): string {
  let xml = '';
...
export function scoreToMusicXML(score: Score, options?: SerializeOptions): string {
...
`;
  ...
  for (let mi = 0; mi < score.measures.length; mi++) {
    ...
    const measureStart = mi * MEASURE_DIVISIONS;
...
export function scoreToMusicXMLMultiPart(
  score: Score,
  partSpecs: PartSpec[],
