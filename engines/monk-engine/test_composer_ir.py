...
def test_valid_composer_ir():
    ir = ComposerIR(title="Test", section_order=["primary", "return"], section_roles={"primary": SectionPlan(role="primary", bar_count=8), "return": SectionPlan(role="return", bar_count=8)})
...
def test_invalid_no_primary_or_return():
    ...
    assert not validate_composer_ir(ir).valid
...
def test_section_roles_match_order():
    ...
    assert not validate_form_plan(ir).valid
