...
def test_generate_from_title_deterministic():
    a = generate_composer_ir_from_title("Blues Head", seed=42)
    b = generate_composer_ir_from_title("Blues Head", seed=42)
    assert a.title == b.title
    assert a.section_order == b.section_order


def test_generate_from_premise():
    ir = generate_composer_ir_from_premise("Angular motif", seed=0)
    assert ir.title
