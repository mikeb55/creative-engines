...
exports.generateWybleScore = generateWybleScore;
...
function getChordForBar(progression, barIndex) {
    let acc = 0;
    for (const seg of progression) {
        if (barIndex < acc + seg.bars)
            return parseChord(seg.chord);
        acc += seg.bars;
    }
...
function pickPitch(candidates, last, min, max) {
    ...
    const choice = (stepwise.length > 0 ? stepwise : pool).reduce((a, c) => (Math.abs(c - last) < Math.abs(a - last) ? c : a));
    return clamp(choice, min, max);
}
/** Generate Wyble etude as Score using core measure builder. */
function generateWybleScore(progression, options) {
    ...
    for (let i = 0; i < bars; i++) {
        const measure = (0, measureBuilder_1.createMeasure)(i, [1, 2]);
        const v1 = { pos: 0 };
        const v2 = { pos: 0 };
        ...
        lastLower = bassPitch;
        (0, measureBuilder_1.pushNote)(measure, 1, melodyPitch, 16, v1);
        (0, measureBuilder_1.pushNote)(measure, 2, bassPitch, 16, v2);
