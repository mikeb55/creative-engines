...
const baselineMusicXMLWriter_1 = require("../../tools/baseline_musicxml_test/baselineMusicXMLWriter");
const DIVISIONS = 4;
const MEASURE_DURATION = 16;
const MEASURE_COUNT = 8;
/** Simple chord tones: Dm7, G7, Cmaj7 over 8 bars. */
const CHORD_ROOTS = [62, 62, 67, 67, 60, 60, 60, 60]; // D, D, G, G, C, C, C, C
const MELODY_TONES = [0, 2, 4, 5, 7, 9, 11]; // scale degrees
const BASS_TONES = [0, 5, 7]; // root, fifth, seventh
function getMelodyPitches(barIndex) {
...
function getBassPitches(barIndex) {
    const root = CHORD_ROOTS[barIndex];
    return [
...
function validate(measures) {
    ...
    for (let i = 0; i < measures.length; i++) {
        ...
        const v1Total = 4 * 4;
        const v2Total = 8 * 2;
        if (m.voice1.length !== 4)
            errors.push(`Measure ${i + 1} voice1: expected 4 notes, got ${m.voice1.length}`);
        if (m.voice2.length !== 2)
            errors.push(`Measure ${i + 1} voice2: expected 2 notes, got ${m.voice2.length}`);
        if (v1Total !== MEASURE_DURATION)
            errors.push(`Measure ${i + 1} voice1: expected 16 divisions`);
        if (v2Total !== MEASURE_DURATION)
            errors.push(`Measure ${i + 1} voice2: expected 16 divisions`);
    }
    return { passed: errors.length === 0, errors };
}
const wybleCleanDir = path.join(paths_1.PATHS.wyble, 'clean');
const outputPath = path.join(wybleCleanDir, 'wyble_clean.musicxml');
function main() {
    (0, paths_1.ensureDir)(wybleCleanDir);
    ...
    fs.writeFileSync(path.join(wybleCleanDir, 'validation_report.json'), JSON.stringify(report, null, 2), 'utf8');
    if (!validation.passed) {
        console.error('Validation failed:', validation.errors);
        process.exit(1);
    }
    const xml = (0, baselineMusicXMLWriter_1.writeTwoVoiceMusicXML)(measures);
