...
exports.parseMusicXMLToProgression = parseMusicXMLToProgression;
const musicxmlValidation_1 = require("./musicxmlValidation");
function parseChordFromHarmony(rootStep, rootAlter, kind) {
    const alterStr = rootAlter === 1 ? '#' : rootAlter === -1 ? 'b' : '';
    const root = rootStep + alterStr;
    const k = (kind || '').toLowerCase();
    if (k.includes('major') || k.includes('maj') || k === '')
        return root + 'maj7';
    if ((k.includes('dominant') || k === '7') && !k.includes('minor'))
        return root + '7';
    if (k.includes('minor') || k.includes('min'))
        return root + 'm7';
    if (k.includes('dim'))
        return root + 'dim';
    if (k.includes('halfdim') || k.includes('m7b5'))
        return root + 'm7b5';
    if (k.includes('aug'))
        return root + 'aug';
    return root + '7';
}
function extractHarmoniesFromMeasure(measureXml) {
    const chords = [];
...
function coalesceProgression(measureChords) {
    const result = [];
...
function parseMusicXMLToProgression(xml) {
    const validationError = (0, musicxmlValidation_1.validateMusicXMLContent)(xml);
    if (validationError)
