...
function phase1_EngineLoad(): void {
  try {
    if (typeof generateEllingtonOrchestration !== 'function') {
      throw new Error('generateEllingtonOrchestration is not a function');
    }
    generateEllingtonOrchestration(PROGRESSIONS.ii_v_i, 1);
  } catch (e) {
...
function phase2_Generation(): void {
  const names = ['ii_v_i', 'blues', 'rhythm_changes'];
  for (const name of names) {
    const prog = PROGRESSIONS[name];
    const orch = generateEllingtonOrchestration(prog, 1);
...
function phase3_VoiceLeading(): void {
  const orch = generateEllingtonOrchestration(PROGRESSIONS.ii_v_i, 1);
  ...
  for (const section of [orch.trumpets, orch.trombones, orch.saxes]) {
    ...
    for (const v of section) {
      ...
      if (prev.length > 0 && v.pitches.length > 0) {
        ...
        if (maxJump > 24) violations.push(`Voice leading jump ${maxJump} semitones`);
      }
      prev = v.pitches;
    }
  }
...
function phase4_MusicXMLExport(): void {
  const orch = generateEllingtonOrchestration(PROGRESSIONS.ii_v_i, 1);
  const xml = exportOrchestrationToMusicXML(orch);
...
function phase5_DesktopGenerator(): void {
  let result: { runFolderPath?: string; error?: string };
  try {
    const child = spawnSync(
      'npx',
      ['ts-node', '--project', 'tsconfig.json', 'ellingtonDesktopGenerate.ts', 'ii_V_I_major', 'classic'],
      { cwd: engineDir, shell: true, encoding: 'utf-8', timeout: 60000 }
    );
...
function phase6_Iteration(): void {
  const CANDIDATE_COUNT = 40;
  const EXPORT_COUNT = 3;

  function score(orch: { trumpets: { pitches: number[] }[]; trombones: { pitches: number[] }[]; saxes: { pitches: number[] }[]; rhythm: { pitches: number[] }[] }): number {
    let s = 10;
    for (const section of [orch.trumpets, orch.trombones, orch.saxes]) {
      let prev: number[] = [];
      for (const v of section) {
  ...
  const candidates: { orch: unknown; score: number }[] = [];
  for (let i = 0; i < CANDIDATE_COUNT; i++) {
    const orch = generateEllingtonOrchestration(PROGRESSIONS.ii_v_i, Date.now() + i * 13);
