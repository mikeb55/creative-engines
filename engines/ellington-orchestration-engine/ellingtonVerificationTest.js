...
const OUT_ROOT = path.join(path.resolve(__dirname, '..', '..'), 'apps', 'ellington-orchestration-desktop', 'outputs', 'ellington');
const TEST_1_II_V_I = [
    { chord: 'Dm7', bars: 2 },
    { chord: 'G7', bars: 2 },
    { chord: 'Cmaj7', bars: 4 },
...
function scoreBackgroundSupport(plan) {
    let violations = 0;
    for (const b of plan.bars) {
...
function validateOrchestrationLogic(plan) {
    const errors = [];
    for (const b of plan.bars) {
    ...
    if (densities.size === 1)
        errors.push('Density does not vary across phrases');
    const contrastBars = plan.bars.filter((b) => b.contrastMode !== 'combined' || b.callResponse !== 'none');
    if (contrastBars.length === 0)
        errors.push('No brass/reed contrast in plan');
    return errors;
}
...
function runTest(name, progression, seedBase, params) {
    const candidates = [];
    for (let i = 0; i < 20; i++) {
    ...
    for (let i = 0; i < toExport.length; i++) {
        ...
        fs.writeFileSync(path.join(runPath, `ellington_plan_rank${rank}.md`), planToMarkdown(plan), 'utf-8');
        fs.writeFileSync(path.join(runPath, `ellington_plan_rank${rank}.json`), JSON.stringify(plan, null, 2), 'utf-8');
    }
