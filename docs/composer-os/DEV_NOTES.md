...
- **Key signature (V3.4):** **`core/harmony/keyInference.ts`** runs **after** score generation; **`applyKeySignatureToScoreAndContext`** attaches **`score.keySignature`** and **`generationMetadata.keySignatureReceipt`** before MusicXML export. Harmony generation is unchanged. **`keySignatureMode`** on **`runGoldenPath`** / **`GenerateRequest`**: **`auto`** (infer), **`override`** (parse **`tonalCenterOverride`** or **`tonalCenter`**), **`none`** (hide key). Exporter writes **`<key><fifths>`** + **`<mode>`** and optional **`print-object="no"`**.
