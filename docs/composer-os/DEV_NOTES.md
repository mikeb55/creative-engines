...
- **Key signature (V3.4 / V3.4b):** **`core/harmony/keyInference.ts`** runs **after** score generation; **`applyKeySignatureToScoreAndContext`** attaches **`score.keySignature`** and **`generationMetadata.keySignatureReceipt`** before MusicXML export. Harmony generation is unchanged. **`keySignatureMode`** on **`runGoldenPath`** / **`GenerateRequest`**: **`auto`** (infer), **`override`** (parse **`tonalCenterOverride`** or **`tonalCenter`**), **`none`** (hide key). Exporter writes **`<key><fifths>`** + **`<mode>`** and optional **`print-object="no"`**. **V3.4b:** natural **minor** key signatures use a fixed **12× fifths** map (e.g. **Bb minor → −5**, not C# +7); **major** uses flat vs sharp **fifth** choice from **flat bias** on chord roots. Receipt fields include **`exportKeyWritten`**, **`inferredKey`**, **`keySignatureModeApplied`**. **Sibelius check:** generate duo with custom 8-bar progression `Bbm9 | Gbmaj7/Bb | …` (see tests) — exported XML must show **five flats** visibly, not C major.
- **Guitar–Bass Duo long-form (V4 Prompt 1/8 — Phase 1/8):** **`core/form/longFormRouteResolver.ts`** gates **`duo32`** vs **`standard8`**. **`buildDuoLongFormCompositionContext`** (`buildLongFormFromDuoSections.ts`) builds 32-bar **`CompositionContext`** without replacing the 8-bar builder. **`core/modulation/`** supplies section-aware **plans** only (not a second harmony engine). **`runGoldenPath`** selects long-form plans when **`totalBars === 32`** and duo preset; **`evaluateDuoLongFormQuality`** adds a long-form soft layer; duo **LOCK** behaviour gates still score **first 8 bars** for extended scores. Remaining V4 master plan: **2/8–8/8** (Slonimsky, Wyble/counterpoint, songwriting completion, quartet + guitar, big band, profiles/style, full LOCK).
