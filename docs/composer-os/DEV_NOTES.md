...
- **Easy wins (usability pack):** **`core/presets-plus/`** — named preset aliases + merge into `GenerateRequest` / `runSongMode` / `runBigBandMode` inputs (no duplicate base presets). **`core/sessions/`** — local JSON session files. **`core/candidates/`** — optional multi-seed `generateComposition` ranking. **`core/diagnostics/`** — `buildDiagnosticsBundle` for UI copy. **`core/lead-sheet/`** — `UniversalLeadSheet` built from `CompositionContext` (Duo/ECM) or Song `LeadSheetContract`.
- **Big Band planning (Prompt 5/7; Prompt 5.6/7 research rules)**: `core/big-band/` — `runBigBandMode` loads **`data/BigBandResearch.md`** (or `COMPOSER_OS_BIG_BAND_RESEARCH`), parses ENGINE RULES into structured lists, merges **`bigBandRuleRegistry`**, resolves **era** (`swing` | `bebop` | `post_bop` | `contemporary`; default **`post_bop`**) and optional **composer** (`ellington` | `basie` | `thad` | `schneider`), applies **`applyBigBandRules`** → `enhancedPlanning`, **`bebopLinePlanner`** when era is bebop, then **`bigBandResearchDrivenValidation`**. Still planning-only JSON — no full big-band MusicXML or voicing engine. See `BIG_BAND_INTEGRATION_NOTES.md`.
...
- **Song Mode (Prompt 3/7; Prompt 6.5/7 research rules; Prompt 7/7 app wiring)**: `runSongMode.ts` loads **`data/Songwriting.md`** (or `COMPOSER_OS_SONGWRITING_RESEARCH`), parses ENGINE RULES, merges **`songwriterRuleRegistry`**, resolves **songwriter style(s)** (default **`beatles`**) and optional **author** / **classical** overlays, runs **hook / chorus / melody behaviour** planners, then **`applySongwritingRules`** → `compiledSong.songwriting` + lead-sheet **`songwritingHints`**. `song_mode_compile` in module registry. **App generate** writes structural JSON — not golden-path MusicXML. See `SONG_MODE_INTEGRATION_NOTES.md`.
