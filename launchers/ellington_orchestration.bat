...
echo [BOOT] %date% %time% ellington_orchestration.bat started >> "%LOGDIR%\bootstrap.log"
...
for /f "delims=" %%i in ('dir /b /s /o-d "%REPO%\outputs\ellington\*.musicxml" 2^>nul') do (
  start "" "%%i"
  goto :opened
)
start "" "%REPO%\outputs\ellington"
:opened
exit /b 0
